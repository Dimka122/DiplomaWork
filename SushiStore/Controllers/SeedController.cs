    ...
    public class SeedController : Controller
    {
        ...
        public IActionResult CreateSeedData(int count)
        {
            ...
            if (count > 0)
            {
                ...
                SET @detail = (RAND() * @rprice) + @rprice;
                INSERT INTO Products (Name,CategoryId,Detail,RetailPrice)
                VALUES (CONCAT('Product',@i,'-',@j),@catId,@detail,@rprice)
                SET @j = @j + 1
                END
                SET @i = @i + 1
                END
